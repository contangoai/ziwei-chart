...
若要啟用站方提供的共用 DeepSeek 額度，請在 **Settings → Variables and Secrets**
新增 `DEEPSEEK_API_KEY`（Secret，切勿以 `VITE_` 開頭）。未填寫 Key 的 DeepSeek
使用者會透過 `/api/chat` Pages Function 使用，Key 只存在伺服器端、不會進入瀏覽器。
